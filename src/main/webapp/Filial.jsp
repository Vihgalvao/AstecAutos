<%-- 
    Document   : Home
    Created on : 17/04/2018, 12:37:51
    Author     : victor.galvao
--%>

<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>JSP Page</title>
    </head>
    <body>
        <h1>Filias</h1><p>
        <ul>
            <li><h3><a href="http://localhost:8080/agendaweb-1.0-SNAPSHOT/form-filial-cadastro.jsp"> Cadastro </a></h3></li>
            <li><h3><a href="http://localhost:8080/agendaweb-1.0-SNAPSHOT/BuscarFilial.jsp"> Atualizar </a></h3></li>
            <li><h3><a href="http://localhost:8080/agendaweb-1.0-SNAPSHOT/BuscarFilialDelete.jsp"> Apagar </a></h3></li>
            <li><h3><a href="http://localhost:8080/agendaweb-1.0-SNAPSHOT/ListarFilial.jsp"> Listar Todas </a></h3></li>
        </ul>  
    </body>
</html>
